...
from fooof_PLA.core.items import OBJ_DESC
from fooof_PLA.core.info import get_indices
from fooof_PLA.core.io import save_fm, load_json
from fooof_PLA.core.reports import save_report_fm
from fooof_PLA.core.modutils import copy_doc_func_to_method
from fooof_PLA.core.utils import group_three, check_array_dim
from fooof_PLA.core.funcs import *
from fooof_PLA.core.errors import (FitError, NoModelError, DataError,
                               NoDataError, InconsistentDataError)
from fooof_PLA.core.strings import (gen_settings_str, gen_results_fm_str,
                                gen_issue_str, gen_width_warning_str)

from fooof_PLA.plts.fm import plot_fm
from fooof_PLA.plts.style import style_spectrum_plot
from fooof_PLA.utils.data import trim_spectrum
from fooof_PLA.utils.params import compute_gauss_std
from fooof_PLA.data import FOOOFResults, FOOOFSettings, FOOOFMetaData
from fooof_PLA.sim.gen import gen_freqs, gen_aperiodic, gen_periodic, gen_model
...
class FOOOF():
    ...
    def aperiodic_regression_fixed(self, func, X, y, guess, reg_weight = 1):
        """
        args:
            func: aperiodic model
            X: frequencies vector (1D vector)
            y: Observed power spectra (1D vector)
            guess: initial guess of parameters (1d, length 2)
            reg_weight: weight of regularization term - negative AUC
    ...
    def _simple_ap_fit(self, freqs, power_spectrum):
        ...
        try:
            with warnings.catch_warnings():
                ...
                if not self.fixed: # full optimization, new FOOOF
                    aperiodic_params = self.aperiodic_regression(mod_expo_function,
                                                    freqs, power_spectrum, np.array(guess),
                                                    self.regularization_weight)

                elif self.fixed: # only optimizing some, new FOOOF
                    optimized_params = self.aperiodic_regression_fixed(get_fixed_mod_func(self.offset, self.knee, self.exp),
                                                    freqs, power_spectrum, guess,
                                                    self.regularization_weight)
    ...
    def _fit_peak(self, flat_iter):
        ...
        return guess_freq, guess_height, guess_std
    ...
    def _fit_peaks(self, flat_iter):
        ...
        while len(guess) < self.max_n_peaks:
            guess_params = self._fit_peak(flat_iter)
            if guess_params == None:
                break
            else:
                guess_freq, guess_height, guess_std = guess_params
    ...
    def _fit_peak_guess(self, guess):
        ...
        lo_bound = np.ndarray.flatten(np.array([bound if bound[0] > self.freq_range[0] else \
            [self.freq_range[0], *bound[1:]] for bound in lo_bound]))
        hi_bound = np.ndarray.flatten(np.array([bound if bound[0] < self.freq_range[1] else \
            [self.freq_range[1], *bound[1:]] for bound in hi_bound]))
        
        gaus_param_bounds = []
        for i in range(len(lo_bound)):
            lo_bound_oi = lo_bound[i]
            hi_bound_oi = hi_bound[i]
        ...
        try:
            gaussian_params = self.gaussian_regression(self.freqs, self._spectrum_flat, 
                                                        list(guess), self.regularization_weight, tuple(gaus_param_bounds),)
    ...
    def _regenerate_model(self):
        ...
        self.fooofed_spectrum_, self._peak_fit, self._ap_fit = gen_model(
            self.freqs, self.aperiodic_params_, self.gaussian_params_, mod=self.mod, return_components=True)
